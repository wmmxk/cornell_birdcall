- 1.preprocessing 
  - extract spectrogram, with learnable parameters
    ...
    - STFT
      ...
      -> apply conv1d on the raw audios, two pass, one for real, one for imaginary.
         ...
         [10, 513, 3001], [10, 513, 3001], [batch_size, number of filters, # of segment]
  ...
  - extract logmel, not learnable    
    ...
    -> expand the 2nd dimension, [10, 3, 3001, 64], Up to now, you can think of each audio has been converted to an image of three repeated channels.
                                                    3001 is # of  frame, 64 is # of frequency.
...
- 3.attention block
    -> norm_att, attention layer is a conv1d layer+softmax, attention is along the last dimension, which is frame
       264 attention heads are calculated from the input of [10, 1024, 93], generating attention weights of [10, 264, 93].
    -> cla, the same input to attention is passed to conv1d+sigmoid, [10, 264, 93], each of a probability that a segment has a type of bird
...
-4. interpolate, padding ,transpose
    - segmentwise, framewise
      ...
      reshape -> [1, 10, 3001, 264], [batch size, # of TTA, frame, # of classes]
    - clipwise
      reshape -> [1, 10, 264]

-5  extract raw framewise prediction, and save clipwise prediction. The output is the event, when a bird starts singing and stops singing.
    It should be converted to the predictions used on the leaderboard, a label for each 5-second audio.
    - framewise outputs
      average over TTA, [3001, 264], the first dimension is 1, does not matter
      compare it with threshold, generating a binary outputs, [3001, 264]
      analyze each column using a while loop, 
        - Initialization of the while loop: find the indices of all frames that have a bird via np.argwhere(),
                                            returns a list of indices (detected) of True, each index represents a frame
        - merge adjacent true indices:
          - head_idx and tail_idx both start from 0
          - check detected[tail_idx+1] - detected[tail_idx] == 1 or not.
            - if not, means the next frame that has a bird is not connected to the current frame
              - extract a segment
...
-6 post-processing. map each event to one or multiple 5-second clips. If the event is short, it is mapped to one 5-second clip,
   If it is a long event, it is mapped to multiple clips.
   Assume there is only one model. prediction_df holds all the events in all the testing audios. So group by by "audio id" first,
     so that you can convert event to the prediction of each 5-second clip of this audio.
     
   - it is possible that there are multiple birdcalls at one point, so it is set not one value, 
...
7. Ensemble:
   TTA ensemble has been done when generating clipwise/segmentwise prediction.
   This step is ensemble the events detected by all the models.
   - get a list of the ids of all the unique clips that have a bird and no bird.
   - 
